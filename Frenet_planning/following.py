...
def calc_following_path(c_d, c_d_d, c_d_dd, s0, s0_d, s0_dd, s1, s1_d, s1_dd):
    ...
    for di in np.arange(MIN_ROAD_WIDTH, MAX_ROAD_WIDTH, D_ROAD_W):
        ...
        for Ti in np.arange(2.0, 5.0, DT):
            fp = FrenetPath()
            ...
            for delta_s in [s1 - s0]:
                ...
                tfp.cd = K_J * Jp + K_T * 10 * Ti + K_D * tfp.d[-1] ** 2
                tfp.cv = K_J * Js + K_T * 10 * Ti + K_D * ds
...
def main():
    ...
    car_1 = Vehicle(csp, 30.0, 0.0, 8.0 / 3.6)
    car_2 = Vehicle(csp, 0.0, 6.8, 10.0 / 3.6)
    cars = [car_1, car_2]

    mode = Mode.VELOCITY_KEEPING

    for i in range(SIM_LOOP):
        if mode != Mode.FOLLOWING:
            mode = decision(s0, car_1.s() - D0 - tau * car_1.s_d())
        
        if mode == Mode.FOLLOWING:
            path, all_paths = frenet_following_optimal_planning(
                csp, s0, s0_d, s0_dd, c_d, c_d_d, c_d_dd, car_1.s() - D0 - tau * car_1.s_d(), car_1.s_d(), car_1.s_dd(), ob)
        else:
        ...
        if show_animation:  # pragma: no cover
            ...
            # vehicle = plt.Rectangle(car.position(), car.length(), car.width(), angle = car.yaw(), color = '#444444')
            # plt.gca().add_patch(vehicle)

            for car in cars:
                if car.rect() is not None:
                    plt.plot(car.rect()[:, 0],  car.rect()[:, 1], linestyle = '-', color = '#666666')
            for i in range(0, len(all_paths), 1):
                all_path = all_paths[i]
                plt.plot(all_path.x[1:], all_path.y[1:], linestyle = '-', marker = 'o', markersize = 1.0, color = '#228800')
            ...
            plt.title("v[km/h]:" + str(c_speed * 3.6)[0:4] + ", a:" + str(s0_dd)[0:4])
            plt.grid(True)
            plt.pause(0.0001)
